...
- device-linking OTP and email fallback authority
...
- the user's email account during bootstrap and email fallback
...
The attacker can receive an email fallback OTP. Because version 1 has no MFA or recovery code, email control is sufficient to add or reactivate a device on the account. Existing devices remain unchanged.
...
| Device enrollment abuse | OTP generated by an active device, short expiry, one-time consumption, proof of possession, attempt limit | Anyone who obtains a live OTP can add a device before it expires |
...
| TURN theft | Credentials issued only for an authenticated active-device transfer, short TTL, transfer binding, issuance quotas | A stolen credential can be used until expiry |
...
The backend applies separate limits to email OTP requests, device-linking OTP creation and redemption, challenge issuance, WebSocket connections, transfer offers, signaling messages, and TURN credentials. Limits use both account and network signals where practical. Error messages do not reveal whether an email or device exists.

Message bodies have small fixed limits. WebSocket queues are bounded, stale offers expire after 10 minutes, and an inactive socket is closed after missed heartbeats. A user cannot obtain TURN credentials for a cross-account, inactive-device, or expired transfer.
...
- account bootstrap and email-fallback delivery
...
- theft or phishing of a live device-linking OTP
